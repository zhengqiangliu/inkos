...
import { formatAuditDimensionsPreview, formatAuditPriorityPreview } from "./audit-dimensions.js";
...
export function buildWriterSystemPrompt(
  ...
  const auditGateBlock = buildAuditGateBlock(genreProfile, bookRules, isEnglish, {
    chapterNumber,
    chapterPlan,
    hasParentCanon: auditContext?.hasParentCanon ?? false,
    fanficMode: fanficContext?.fanficMode,
  });
...
function buildPreWriteChecklist(book: BookConfig, gp: GenreProfile): string {
  ...
  lines.push(
    "",
    "【写后自审三问（输出正文前必须完成）】",
    "A. OOC复查：主角/关键角色的行为/口吻是否与已有人设一致？有无无铺垫的性格突变？",
    "B. 大纲复查：本章是否仅推进了当前卷纲节点，有无提前消耗后续剧情？（违反→critical -35分）",
    "C. 衔接复查：本章开头与上章结尾在时间/空间/情绪上是否自然衔接？有无无过渡跳切？",
  );
...
function buildAuditDimensionsPreview(
  gp: GenreProfile,
  bookRules: BookRules | null,
  isEnglish: boolean,
  options: {
  ...
  return formatAuditDimensionsPreview(gp, bookRules, isEnglish, options);
}
...
${isEnglish ? "| Risk Forecast | Dimension N (specific risk: ...) → mitigation: ... | Fill ≥1 entry; verify against final draft before output |" : "| 高风险预判 | 维度N（具体风险：……）→ 规避动作：…… | 至少填1条；写完正文后回头对照验证 |"}
| 本章冲突 | 一句话概括 | |
