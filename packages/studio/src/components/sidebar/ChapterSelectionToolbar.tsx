...
export function ChapterSelectionToolbar({
  ...
  const handleModify = useCallback(async () => {
    ...
    try {
      ...
      if (!sessionId) sessionId = createDraftSession(bookId);
      ...
      await sendMessage(sessionId, instruction, bookId, { skipAutoNewPrefix: true });
      setBrief("");
    } finally {
      setSending(false);
    }
  ...
  return (
    <div className={`absolute top-12 right-2 z-20 w-[260px] overflow-hidden rounded-xl border shadow-2xl ring-1 ring-black/5 ${modeMeta.panelClassName}`}>
      <div className="flex items-center justify-between gap-2 border-b border-border/25 bg-background/65 px-3 py-2">
        <div className="flex min-w-0 items-center gap-2">
          <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-medium tracking-wide ${modeMeta.chipClassName}`}>
            {selectionModeActive ? "AI 选择模式" : modeMeta.label}
          </span>
          {(hasSelection || waitingForSelection || selectionModeActive) && (
            <span className="tabular-nums text-[10px] text-muted-foreground/60">
              {hasSelection ? `${charCount} 字` : "等待选区"}
            </span>
          )}
        </div>
        {(hasSelection || waitingForSelection || selectionModeActive) && (
          <button
            type="button"
            onClick={onDismiss}
            className="shrink-0 rounded-md p-0.5 text-muted-foreground transition-colors hover:bg-secondary/50 hover:text-foreground"
      ...
      <div className={`h-1 ${hasSelection ? "bg-primary/70" : selectionModeActive ? "bg-amber-500/70" : "bg-border/60"}`} />
      ...
      <div className="px-3 pt-2 pb-1">
        <div className={`rounded-lg border px-2.5 py-2 ${hasSelection ? "border-primary/25 bg-primary/10" : "border-border/25 bg-background/65"}`}>
          <div className="mb-1 flex items-center justify-between gap-2">
            <span className={`text-[10px] uppercase tracking-wider ${hasSelection || waitingForSelection || selectionModeActive ? "text-primary/80" : "text-muted-foreground/70"}`}>
              {waitingForSelection ? "等待正文选中" : hasSelection ? "选中内容" : selectionModeActive ? "选择模式" : "全文说明"}
            </span>
            {hasSelection && (
              <span className="tabular-nums text-[10px] text-muted-foreground/60">{charCount} 字</span>
            )}
          </div>
          <p className={`text-[11px] leading-5 ${hasSelection || waitingForSelection || selectionModeActive ? "text-foreground/95" : "text-muted-foreground/80"}`}>
            {waitingForSelection
              ? "请在正文中拖选需要修改的片段，选区会在这里同步显示。"
              : hasSelection
                ? truncatedPreview
                : selectionModeActive
                  ? "正在等待正文选区，面板将自动切换为选中模式。"
                  : "未选中文本时，面板将自动按全文修改。"}
          </p>
        </div>
      </div>
      ...
      <div className="px-3 pb-2">
        <textarea
          ...
          disabled={sending}
          rows={2}
          className="w-full resize-none rounded-lg border border-border/40 bg-background/85 px-2.5 py-1.5 text-[11px] leading-5 outline-none transition-colors focus:border-primary/40 disabled:opacity-50"
        />
      </div>
      ...
      <div className="flex items-center justify-between px-3 pb-2.5">
        ...
        <button
          ...
          disabled={!canModify}
          className="ml-auto inline-flex items-center gap-1 rounded-lg bg-primary/10 px-2.5 py-1 text-[11px] text-primary transition-colors hover:bg-primary/20 disabled:opacity-40"
        >
