...
interface DirectorPlanHistoryModalProps {
  readonly bookId: string;
  readonly currentPlan: DirectorPlan;
  readonly onClose: () => void;
  readonly onRestore: (plan: DirectorPlan) => void;
}
...
function DirectorPlanSummary({ plan }: { plan: DirectorPlan }) {
  return (
    <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-4">
      <div className="rounded-xl border border-border/20 bg-background/60 px-3 py-2">
        <div className="text-[10px] text-muted-foreground">视觉声明</div>
        <div className="mt-1 line-clamp-3 text-[11px] text-foreground">{plan.visualStatement || "无"}</div>
      </div>
      <div className="rounded-xl border border-border/20 bg-background/60 px-3 py-2">
        <div className="text-[10px] text-muted-foreground">导演意图</div>
        <div className="mt-1 line-clamp-3 text-[11px] text-foreground">{plan.directorIntent || "无"}</div>
      </div>
      <div className="rounded-xl border border-border/20 bg-background/60 px-3 py-2">
        <div className="text-[10px] text-muted-foreground">规则数</div>
        <div className="mt-1 text-[11px] text-foreground">
          视觉 {plan.visualRules.length} / 运镜 {plan.cameraRules.length}
        </div>
      </div>
      <div className="rounded-xl border border-border/20 bg-background/60 px-3 py-2">
        <div className="text-[10px] text-muted-foreground">分集规划</div>
        <div className="mt-1 text-[11px] text-foreground">{plan.episodePlans.length} 集</div>
      </div>
    </div>
  );
}

export function DirectorPlanHistoryModal({
  bookId,
  currentPlan,
  onClose,
  onRestore,
}: DirectorPlanHistoryModalProps) {
  const [history, setHistory] = useState<ReadonlyArray<DirectorPlanHistoryEntry>>([]);
  ...
  const [diffResult, setDiffResult] = useState<DirectorPlanDiffResult | null>(null);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      setError(null);
      try {
        const data = await fetchJson<DirectorPlanHistoryResponse>(`/books/${bookId}/director-plan/history`);
        if (cancelled) return;
  ...
  const handleCompare = async () => {
    ...
    setDiffResult(null);
    try {
      const result = await fetchJson<DirectorPlanDiffResult>(
        `/books/${bookId}/director-plan/diff?fromVersion=${selectedLeft}&toVersion=${selectedRight}`,
      );
  ...
  const handleRollback = async (targetVersion: number) => {
    ...
    try {
      const result = await fetchJson<{ ok: boolean; plan: DirectorPlan }>(
        `/books/${bookId}/director-plan/rollback`,
        {
          method: "POST",
      ...
      <div className="w-[min(1160px,calc(100vw-2rem))] max-h-[88vh] overflow-hidden rounded-2xl border border-border/30 bg-background shadow-2xl">
        <div className="flex items-center justify-between gap-3 border-b border-border/20 px-4 py-3">
          <div className="min-w-0">
            <div className="text-sm font-medium">导演规划版本历史</div>
            <div className="truncate text-[11px] text-muted-foreground">
              当前更新时间 · {formatDate(currentPlan.updatedAt)}
            </div>
          </div>
        ...
        <div className="max-h-[calc(88vh-58px)] overflow-y-auto p-4">
          {loading ? (
            ...
            <div className="py-16 text-center text-xs text-muted-foreground">暂无导演规划版本记录</div>
          ) : viewMode === "compare" ? (
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid gap-3 lg:grid-cols-2">
                    <div className="rounded-2xl border border-border/20 bg-card/30 p-4">
                      ...
                      <DirectorPlanSummary plan={diffResult.from} />
                    </div>
                    <div className="rounded-2xl border border-border/20 bg-card/30 p-4">
                      <div className="mb-3 flex items-center justify-between gap-2">
                      ...
                      <DirectorPlanSummary plan={diffResult.to} />
                    </div>
                  </div>
                </div>
              ) : (
              ...
              <div className="space-y-2">
                {history.slice().reverse().map((entry) => (
                  ...
                  >
                    ...
                    <div className="mt-1 text-[10px] text-muted-foreground">
                      {entry.plan.episodePlans.length} 集 · 视觉规则 {entry.plan.visualRules.length}
                    </div>
                  </button>
                ))}
              </div>
              ...
              <div className="rounded-2xl border border-border/20 bg-card/30 p-4">
                {selectedEntry ? (
                  <div className="space-y-3">
                    ...
                    <DirectorPlanSummary plan={selectedEntry.plan} />
                    <div className="grid gap-3 md:grid-cols-2">
                      <div className="rounded-xl border border-border/20 bg-background/60 p-3">
                        <div className="text-[10px] text-muted-foreground">视觉规则</div>
                        <div className="mt-1 text-[11px] text-foreground">{selectedEntry.plan.visualRules.join("、") || "无"}</div>
                      </div>
                      <div className="rounded-xl border border-border/20 bg-background/60 p-3">
                        <div className="text-[10px] text-muted-foreground">运镜规则</div>
                        <div className="mt-1 text-[11px] text-foreground">{selectedEntry.plan.cameraRules.join("、") || "无"}</div>
                      </div>
                    </div>
                  </div>
                ) : (
