...
export function buildExecutionPanelSummary(
  ...
  for (const execution of executions) {
    earliestStartedAt = earliestStartedAt === undefined ? execution.startedAt : Math.min(earliestStartedAt, execution.startedAt);
    const endedAt = execution.status === "running" || execution.status === "processing"
      ? nowMs
      : (execution.completedAt ?? execution.startedAt);
    latestEndedAt = latestEndedAt === undefined ? endedAt : Math.max(latestEndedAt, endedAt);
  ...
  return {
    ...
    completed,
    failed,
    elapsedMs: earliestStartedAt === undefined || latestEndedAt === undefined ? 0 : Math.max(0, latestEndedAt - earliestStartedAt),
...
export function ExecutionPanel({ executions, collapsed, onCollapsedChange }: ExecutionPanelProps) {
  ...
  const compactStatus = isRunning
    ...
    : summary.failed > 0
      ...
      : `全部完成 · 耗时 ${formatElapsedMs(summary.elapsedMs)}`;

  return (
    <Collapsible open={!collapsed} className="rounded-xl border border-border/60 bg-card shadow-sm">
      <button
        type="button"
      ...
      >
        <div className="flex items-center justify-between gap-2">
          <div className="flex min-w-0 items-center gap-2">
            ...
            {collapsed && <span className="min-w-0 truncate text-xs text-muted-foreground">{compactStatus}</span>}
            {!collapsed && isRunning && (
          ...
          <ChevronDown size={14} className={`shrink-0 text-muted-foreground transition-transform ${collapsed ? "" : "rotate-180"}`} />
        </div>
      </button>
      <CollapsibleContent className="px-2 pb-2">
          ...
          <div className="mb-2 rounded-lg border border-border/50 bg-muted/40 px-3 py-2 text-xs">
            {summary.activeStageLabel && (
              <div className="text-foreground/80">
                当前阶段：<span className="font-medium text-foreground">{summary.activeStageProgressText ?? summary.activeStageLabel}</span>
              </div>
            )}
            {summary.latestFailure && <div className="mt-1 text-destructive">最近失败：{summary.latestFailure}</div>}
            {summary.autoReviewMeta && summary.autoReviewMeta.length > 0 && (
              <div className="mt-1 flex flex-wrap gap-1">
                {summary.autoReviewMeta.map((item) => (
                  <span key={item} className="inline-flex items-center rounded border border-primary/30 bg-primary/10 px-1.5 py-0.5 text-[11px] text-primary">
                    {item}
                  </span>
                ))}
              </div>
        ...
        <div className="mb-2 flex items-center justify-end gap-1 text-[11px]">
          <button
            ...
            className={`rounded border px-2 py-1 transition-colors ${pipelineExpandMode === "expanded" ? "border-primary/50 bg-primary/10 text-primary" : "border-border/50 bg-muted/30 text-muted-foreground hover:bg-muted/50"}`}
          >
            全部展开
          </button>
          <button
            ...
            className={`rounded border px-2 py-1 transition-colors ${pipelineExpandMode === "collapsed" ? "border-primary/50 bg-primary/10 text-primary" : "border-border/50 bg-muted/30 text-muted-foreground hover:bg-muted/50"}`}
          >
            全部折叠
          </button>
          <button
            ...
            className={`rounded border px-2 py-1 transition-colors ${pipelineExpandMode === "auto" ? "border-primary/50 bg-primary/10 text-primary" : "border-border/50 bg-muted/30 text-muted-foreground hover:bg-muted/50"}`}
          >
            自动
          </button>
        </div>
