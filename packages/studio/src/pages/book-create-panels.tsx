...
import type { IntroCandidateLike, ReviewChecklistItem } from "./book-create-state";
...
function IntroSeedSection(props: {
  ...
  readonly introModifyNote: string;
      ...
      {introMode === "manual" ? (
        ...
        <div className="rounded-2xl border border-border/40 bg-background/40 p-4 space-y-3">
          ...
          <div className="grid gap-3 md:grid-cols-[1fr_160px]">
            <div className="rounded-xl border border-border/50 bg-background/70 p-3 text-xs leading-6 text-muted-foreground">先生成候选池，再点候选回填到手工模式。这里不会直接进入右侧确认流程。</div>
          ...
          <button onClick={() => void handleGenerateCandidates()} disabled={loading || creating || introCandidateLoading || !autoGenerateAllowed} className={`rounded-md px-4 py-3 text-sm font-medium ${c.btnPrimary} disabled:opacity-50`}>{introCandidateLoading ? "生成中..." : "生成候选池"}</button>
        </div>
      )}
    </IntroSectionShell>
  );
...
export function IntroPanel(props: {
  ...
  setIntroModifyNote: (value: string) => void;
  ...
  handleManualRevise: (kind: "revise" | "polish") => void;
...
export function WizardActions(props: {
  ...
  return (
    <div className="flex flex-wrap gap-2 pt-1">
      <button onClick={handleDiscard} disabled={creating} className="rounded-md border border-border px-4 py-3 text-sm font-medium text-muted-foreground disabled:opacity-50">丢弃草案</button>
      <button onClick={handleBack} disabled={!canGoBack || creating} className="rounded-md border border-border px-4 py-3 text-sm font-medium text-muted-foreground disabled:opacity-50">上一步</button>
      {!isReview ? <button onClick={handleAdvance} disabled={creating || !canAdvance} className="rounded-md px-4 py-3 text-sm font-medium bg-primary text-primary-foreground disabled:opacity-50">{nextStepTitle ? `保存并进入：${nextStepTitle}` : "已到最后一步"}</button> : null}
